...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .mark { display: grid; width: 38px; height: 38px; place-items: center; border-radius: 12px 12px 12px 3px; background: var(--coral); color: white; font-size: 19px; transform: rotate(-8deg); }
        .brand span:last-child { transform: rotate(0deg); }
        .status { display: flex; align-items: center; gap: 8px; color: var(--muted); font-size: 13px; }
        .status i { width: 8px; height: 8px; border-radius: 50%; background: #51b884; box-shadow: 0 0 0 4px #d9f1e4; }
        .intro { display: grid; grid-template-columns: 1.2fr 0.8fr; gap: 48px; align-items: end; margin-bottom: 42px; }
        ...
        .avatar { display: grid; width: 36px; height: 36px; place-items: center; border-radius: 50%; background: var(--navy); color: white; font-family: 'Space Grotesk', sans-serif; font-size: 13px; }
        ...
        @media (max-width: 720px) {
            ...
            .directory { overflow-x: auto; }
            table { min-width: 660px; }
            .directory-head { padding: 20px; }
...
<body>
    <main class="shell">
        ...
        <section class="summary" aria-label="Directory summary">
            ...
            <span>registered<br>members</span>
        </section>
        ...
        <section class="directory" aria-labelledby="directory-title">
            <header class="directory-head">
                <div>
                    ...
                    <p>People currently in your directory</p>
                </div>
                <span class="count"><?= $user_count ?> total</span>
            </header>
            <?php if (!empty($users)): ?>
                <table>
                    <thead>
                        <tr><th>Member</th><th>Email</th><th>Username</th></tr>
                    </thead>
                    <tbody>
                        <?php foreach ($users as $user): ?>
                            <?php $initials = strtoupper(substr($user['firstname'], 0, 1) . substr($user['lastname'], 0, 1)); ?>
                            <tr>
                                <td><div class="person"><span class="avatar"><?= htmlspecialchars($initials) ?></span><?= htmlspecialchars($user['firstname'] . ' ' . $user['lastname']) ?></div></td>
